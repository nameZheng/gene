	...
	<body>
		<div class="container">
			...
			<div id="main" class="row col-sm-12" style="margin-top:20px;margin-bottom:20px;padding-top:15px;padding-bottom:15px;border:1px solid #d7e3dc;">
				...
				<div class="row col-sm-12" style="margin-top:20px;margin-bottom:20px;">
					<span>View gene information, by entering the gene Ensemble ID</span>
				</div>
				<div class="row col-sm-12">
					...
					<div class="col-sm-4">
						<form id="searchform" action="<%=getServletContext().getContextPath() %>/gene.html">
							...
							<br>
							<input type="text" name="id" class="form-control" placeholder="Please input Gene ID">
							<!-- <br>
							<input type="text" name="name" class="form-control" placeholder="Please input Gene Name"> -->
							<br>
		...
		<script>
			<%-- $("#subm").click(function(){
				...
				$.post("<%=getServletContext().getContextPath() %>/gene",$("#searchform").serialize(),function(data,status){
					if(data.status == 200){
			...
			<%-- $("#subm").click(function(){
				...
				let id = $("input[ name='id' ] ").val();
				let name = $("input[ name='name' ] ").val();
				$.get("<%=getServletContext().getContextPath() %>/gene?kind=human&id=ENSG00000006071&name=ABCC8",function(data,status){
					if(data.status == 200){
						r = JSON.parse(data).data;
						let app = new Vue({
							el: '#results',
